...

{
    ...
    public String recPreOrder(BNode<T> current)
    {
        ...
        return current.getData() + " " + recPreOrder(current.getLeft()) + " " + recPreOrder(current.getRight());
    }
    ...
    /**
     * Method that returns the current binary tree as an Array
     * @return
     */
    @SuppressWarnings("unchecked")
    public T[] toArray()
    {
        ArrayList<T> lstToReturn = new ArrayList<>();
        //Start recursion
        recToArray(root.getLeft(),lstToReturn);
        lstToReturn.add(root.getData());
        recToArray(root.getRight(),lstToReturn);

        //Convert Arraylist to Array
        T[] obaReturn = (T[]) Array.newInstance(root.getData().getClass(),lstToReturn.size());
        lstToReturn.toArray(obaReturn);

        return obaReturn;
    }

    /**
     * Method that goes through the binary tree recursively and adds it to an arraylist.
     */
    private void recToArray(BNode<T> obCurrent, ArrayList<T> lstBinary)
    {
        if (obCurrent == null)
        {
            return;
        ...
        recToArray(obCurrent.getLeft(),lstBinary);
        recToArray(obCurrent.getRight(),lstBinary);

        lstBinary.add(obCurrent.getData());
    }

    /**
     * Method that goes through and checks if binary tree is balanced
     * @return
     */
    ...
    private boolean recIsBalanced(BNode<T> obCurrent)
    {
        return false;
    }
