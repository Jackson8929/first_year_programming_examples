...
public class Utils
{
    ...
    /**
     * Generic method that can be used a utlity method for reading values in from a file that has a csv format.
     * This works as long as the particular class that is accepting the CSV field values implements the CopyIT integerace.
     * @param sFile
     * @param obSample
     * @return
     * @param <T>
     */
    public static <T extends CopyIt<T>> ArrayList<T> genCSVRead1(String sFile, T obSample)
    {
        ...
        try (Scanner obIn = new Scanner(new File(sFile)))
        {
            ...
            {
                String[] saVals = parseCSVLine(obIn.nextLine());
                obSample.setVals(saVals);
                lstReturn.add(obSample.copy());
            }
    ...
    public static <T>T[]convertToArray(ArrayList<T> aVals)
    {
        if (aVals.size() == 0)
        {
            return null;
        ...
        T[] aReturn = (T[]) Array.newInstance(aVals.get(0).getClass(),aVals.size());

        //Transfer all the elements from the arrayList to the array
        aVals.toArray(aReturn);
